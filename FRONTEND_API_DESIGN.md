...
5. `action-options.actors[].features` 会暴露可执行的角色资源特性和怪物模板特性，提交时走 `/actions/use-feature`
6. 当前行动者若是敌方，侧栏只提供 DM 辅助装填与规则执行，不会自动替 DM 做决策
7. actor、装备、法术和怪物摘要优先消费后端返回的 `*_display` 字段，避免把英文 canonical 字段直接渲染到中文界面
...
3. 选择初始剧本
4. 在聊天页直接做最小战斗控制
5. 从 `action-options` 自动带出攻击项、部分可选角色、法术、物品和特性候选
