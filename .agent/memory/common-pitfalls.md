...
- 塔莎狂笑术已由 `cast_spell` 连同明确目标原子结算初次感知豁免；`active_spell_effects` 保存来源、目标、DC、期限及状态所有权。受伤触发优势重掷，回合末正常重掷，不能再用通用豁免工具重复该法术的骰点。失能者跳过行动仍须结算回合末事件，并在自己的回合开始恢复反应额度。
- `cast_spell` 共用可用性、法术位、专注和行动槽逻辑；攻击型法术仍用 `cast_id` 完成后续攻击。塔莎狂笑术必须提供目标并由规则层完成豁免/状态，不把动作已消耗视作效果已结算；维持专注不是重新施法。本次自动持续效果支持范围仅为塔莎狂笑术，其他法术仍按各自已支持的规则工具处理。
...
- 长请求提交必须校验起始 `state_version`；重写/回退使用当前存档版本保护历史快照的提交，不能把历史版本直接当当前版本。所有普通存储写入均受业务版本与暂停守卫约束。
- 暂停的 checkpoint 不能自动吸收外部局部写入。API 与普通存储写入都阻止暂停期间修改本局状态，GET 游戏也不补写选项；继续旧暂停前比较业务基线并忽略版本/时间戳/trace，历史漂移时保留公开存档、终止私有事务。`action_options` 的禁用字段必须与对应 `state_version` 一起消费，避免旧响应继续锁住已结束的暂停。
...
- 躲藏必须经 `hide_actor` 检查场景前提并结算 DC15 敏捷（隐匿），保存独立 `hiding` 来源及发现 DC；通用技能检定不自动授予躲藏。合法失败仍消耗战斗动作，不能为了满足 hostile_attack 再强迫普通角色同回合攻击。
- 2024 突袭是开战时的未察觉事实，只在初次先攻产生劣势；躲藏/隐形给先攻优势，两者抵消，不跳过首回合。`surprised_refs` 必须在开战前确定，已有战斗不能重启先攻。非躲藏的意外开战须有独立场景依据，不能把失败躲藏改成成功，也不授予 Sneak Attack 职业伤害。
- 攻击后、成功的言语施法、敌方发现或明确噪音/暴露会结束躲藏；只清除 `hiding`，不删独立魔法隐形。主动/被动察觉由 `search_hidden` 结算；没有网格视线模型，掩护、警觉与特殊感官是否能实际看见目标仍需明确场景裁定。
- 重试/重写的明确失败使用 `turn_not_committed` SSE 错误，保留原分支并返回已回滚骰点；前端在中断卡片旁显示原因。只有真正缺少服务端最终结论的断线才标记 unknown，不能把所有 catch 都当作提交状态未知。
- 新回合/重试/重写 SSE 的连接和无数据等待均有 45 秒上限；服务端每 10 秒无业务事件时发心跳，收到最终提交载荷即可结束前端等待，不再依赖连接 EOF。断线仍不能证明服务端已取消，必须重新加载确认进度，不能自动重放行动。
- Codex 明确的临时 HTTP 连接失败可重试当前模型请求一次；复用已有工具结果，不重放整回合或已掷骰点，不刷新 deadline。认证、协议与规则错误不属于该重连路径。
- 整个 CLI 回合共享配置的等待预算，工具后的下一次模型调用只能使用剩余时间。修复工具轮次有独立总上限，不能通过每次 `tool_call_rounds + 1` 无限续期。
- 新聊天请求不刷新战斗动作额度。`combat_flow` 固定本次输入的玩家行动者；主动作耗尽后只结算明确声明的合法附赠动作，再通过 `advance_turn` 推进 DM 行动，抵达下一玩家决策点停止。不能继续向已耗尽槽位或下一位玩家索要旧指令的攻击。 先攻进入队员回合时，规则层同步 `active_character_id`，工具 delta 同步发布；不要等到交还玩家后再依靠模型修复角色引用，`primary_character_id` 不随之改变。 DM 每次调用还需拿到当前队员的实时装备、法术和资源；开局一次性的队伍摘要不足以支持后续托管队友行动。
- 带工具调用的公开过程正文不写入 `final_response`；校验失败必须给出失败回复。相同工具错误连续三次且没有成功工具进展时终止并回滚，不能将反复的执行预告当成正式回复或工具成功证据。
...
- 回复长度设置不能只依赖 DM 提示词；正常回合在 `finalize_turn` 提交前用去空白字符数检查，并交给无工具的独立纯文本调用扩写或压缩。长度是展示偏好，后处理未命中只能记录 warning、保留最佳正文，不能回滚已完成回合或向玩家暴露内部校验文案；系统错误说明不受叙事长度约束。 长度策略按本条初始阶段区分：从战斗开始可豁免最低字数，但允许充分描写重要交锋；剧情转战斗仍保留原总长度目标，扩写优先补足进战前剧情。上限与本局偏好保持不变。`ChatMessage.narrative_mode` 仅保存卡片展示类型，不能作为整条最小字数豁免依据。
- 自动行动建议及其免版本缓存写入入口已移除；旧 `ChatMessage`、`AdventureHook` 和 trace 中的建议字段由 schema 忽略，加载或恢复不得触发建议模型调用。
- 主持回复重试必须由服务端根据 assistant 消息索引解析前一条玩家行动和对应 rewind snapshot；重试或提交玩家消息重写后，前端先替换/移除目标及后续消息、旧 interrupt和思考过程，传输失败才恢复整组 UI 投影。失败回复常驻重试入口。
...
- 同步工作副本可能保留较旧文件时间戳，而 Uvicorn StatReload 只响应时间戳增大；文件内容已更新不代表运行实例已经重载。 Windows 无窗口 worker 也可能停在重载等待中并继续响应旧接口；health 200 本身不能证明代码版本已更新。交付时检查实际工具拓扑或 OpenAPI 新字段，优先正常热重载，不能强杀活动游戏回合。
...
- `inventory_narration.py` 根据本轮初始与最终权威物品栏/金币差值生成 `**物品｜…**` 标记；仅成功回合发布。扩写后重新校准，去除伪造数量与重复标记；净数量不变不宣称新增，装备变化单独记录。
- 持续效果清理只能移除该来源引入的状态；重叠效果需移交状态所有权，既有或后加的独立同名状态保留。先确定伤害的致死/非致死结局，再处理效果的受伤与死亡事件；中途出错须回滚整个动作。
- `rules_time_seconds` 是游戏内时间：战斗每轮六秒，非战斗由显式 `advance_time` 推进；玩家等待、暂停或网络重连不推进时钟。`end_concentration` 结束该施法者维持的效果。旧存档仅有专注名称不等于存在已结算目标，不能自动补造状态或骰点。
- 战斗状态栏通过 action options 的 `status_entries` 展示增益、减益、专注及来源说明，必须与当前快照版本一致。未知来源状态保留中性说明，不在浏览器推测持续期、加值或自动解除条件。

- 拾取物品先在副本上补全规则再发布库存；标准装备支持中英文精确名称，外观名称可用 `rules_name` 引用目录。同名但不同环级/效果不可混叠。旧库存经 action-options 只读补全，武器显示与结算共享 RuleCatalog；不在 GET 中自动装备或写存档。治疗/卷轴/投掷物的效果说明不等于 `use_item` 已自动执行这些效果；未知数值不猜测，卷轴保留每发、基础环级和升环限定。
