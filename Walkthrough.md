...
最新进展：已经完成 Phase 1A、Phase 1B、Phase 2A 和 Phase 2B。`backend/agent_tools.py` 已承载框架无关工具层，当前 ADK tool wrapper 已委托到该工具层；`backend/agent.py` 中迁移前的不可达旧闭包代码已经删除；`backend/dm_graph.py` 已加入 LangGraph runner 骨架和 OpenAI-compatible 模型节点，可通过 `CHAT_BACKEND=langgraph` 或 `AGENT_BACKEND=langgraph` 手动切换。
...
当前状态：ADK wrapper 已通过 `_run_agent_tool()` 委托到 `backend/agent_tools.py`。`_build_tools()` 现在只保留薄 wrapper，不再包含迁移前的不可达旧闭包代码。
...
当前状态：Phase 1A 和 Phase 1B 已完成。ADK wrapper 已经调用 `AgentToolService`，迁移前旧闭包参考代码已经删除。
