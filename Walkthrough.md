...
最新进展：已经完成 Phase 1A 和 Phase 2A。`backend/agent_tools.py` 已承载框架无关工具层，当前 ADK tool wrapper 已委托到该工具层；`backend/dm_graph.py` 已加入 LangGraph runner 骨架，但尚未接管 `/turns`。
...
LangGraph workflow 骨架。当前包含 `prepare_turn`、`prepare_context`、`draft_response`、`finalize_turn` 四个节点，用可选导入保护 LangGraph 依赖缺失场景。下一步要把 `draft_response` 替换为真实模型与工具执行循环。
...
LangGraph workflow 定义。当前已落地最小骨架，但还没有接管 `/api/v1/games/{game_id}/turns`。
...
2. 完成 LangGraph 单回合 runner。
   - 安装 `langgraph`、`langchain`、`langchain-openai`。
   - 把 `draft_response` 替换为 OpenAI-compatible 模型节点。
