...
LangGraph workflow 定义。当前可通过 `CHAT_BACKEND=langgraph` 或 `AGENT_BACKEND=langgraph` 手动接管 `/api/v1/games/{game_id}/turns`。默认仍是 ADK，因为 LangGraph 路径还需要更多真实回合 smoke test 后再切默认。
