...
- `backend/prompts.py`
  - `build_dm_instruction()` 现在显式接收 workflow phase 指南，不再只依赖 state summary。
- `tests/test_dm_graph_workflow.py`
  - 已补 phase workflow 回归测试，覆盖未完成 setup 的冒险选择回退、combat 强制恢复、level-up 工具约束，以及 prompt 注入检查。

这意味着下一轮如果继续完善 Agent 工作流，优先级应该从“先补 phase policy”转向“把规划器/执行器分成两段式回合”和“整章级回放测试”。
