...
最新进展：已经完成 Phase 1A、Phase 2A 和 Phase 2B。`backend/agent_tools.py` 已承载框架无关工具层，当前 ADK tool wrapper 已委托到该工具层；`backend/dm_graph.py` 已加入 LangGraph runner 骨架和 OpenAI-compatible 模型节点，可通过 `CHAT_BACKEND=langgraph` 或 `AGENT_BACKEND=langgraph` 手动切换。
...
当前状态：ADK wrapper 已通过 `_run_agent_tool()` 委托到 `backend/agent_tools.py`。`_build_tools()` 中早返回之后仍保留迁移前旧闭包代码，等 LangGraph 等价链路跑通后再删除。

`backend/agent_tools.py`

框架无关 Agent 工具层，当前包含 `AgentToolService` 与 `AgentToolExecution`。它接收显式 `GameState` 和工具参数，返回工具结果、时间线事件和状态 delta，不依赖 ADK `ToolContext`。

`backend/dm_graph.py`

LangGraph workflow。当前包含 `prepare_turn`、`prepare_context`、`draft_response`、`finalize_turn` 四个节点，用可选导入保护 LangGraph 依赖缺失场景。`draft_response` 在 `enable_model=True` 时会调用 OpenAI-compatible `ChatOpenAI`，下一步要接入工具执行循环。
...
LangGraph workflow 定义。当前可通过 `CHAT_BACKEND=langgraph` 或 `AGENT_BACKEND=langgraph` 手动接管 `/api/v1/games/{game_id}/turns`，但默认仍是 ADK，因为工具调用节点尚未接入。
...
当前状态：Phase 1A 已完成。ADK wrapper 已经调用 `AgentToolService`，但旧闭包参考代码还没有删除。
...
当前状态：Phase 2A 已完成。`DMGraphRunner` 已有最小 workflow 骨架，后续需要安装依赖并接入真实模型节点、工具执行节点和运行时切换。

补充状态：Phase 2B 已完成。依赖已声明并安装到本地 `DM_Agent` conda 环境，`DMGraphRunner` 已能创建真实模型节点，`DMAgent` 已支持环境变量切换。下一步是工具执行节点。
...
1. **继续完成 Phase 1B：清理迁移前旧闭包代码。**
   - 确认 `AgentToolService` 覆盖所有 ADK 工具。
   - 删除 `_build_tools()` 早返回之后的旧工具闭包参考。
   - 保持 ADK 旧链路可运行。
2. 完成 LangGraph 单回合 runner。
   - 接入 `AgentToolService` 的工具调用循环。
   - 增加阶段化工具白名单。
   - `TurnResult` 响应保持兼容。
3. 做阶段路由和工具白名单。
   - 探索、战斗、升级拆开。
