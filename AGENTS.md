...
以上顺序用于核对项目事实；它不改变系统、开发者、用户指令之间的优先级。
...
- `backend/stealth_rules.py`：2024 躲藏、主动/被动发现、来源状态与优劣势合并。
...
- 本文件保存项目特有约束；个人模型、推理级别和权限放在用户 Codex 配置或当前会话中，不把模型内置提示词复制到仓库。
- `.agent/memory/` 是按需读取的项目文档；自动发现的仓库技能位于 `.agents/skills/<name>/SKILL.md`，两者用途不同。
- 技能应只处理一个明确工作流，写清触发与不适用范围，按需加载正文和引用；使用目录提供的实际路径，避免重复安装同名技能。
- 模型、工具名、Hook 事件和配置键可能随客户端变化；升级时核对当前官方文档和本机实际能力，不把可选或实验功能当作必需条件。
...
- 内容哈希计算前先排除隐私数据、环境文件、文档和技能资产；触发证据与去重指纹使用相同的有效文件集合。
...
项目 hook 新增或变化后，用户需要在 Codex CLI 使用 `/hooks` 审查并信任其当前哈希；项目可信与单个 Hook 启用/信任是不同条件。不要通过写入信任记录或绕过信任的启动参数代替审查。Hook 回归命令为 `python -m unittest discover -s .codex/hooks/tests -v`，不需要游戏运行依赖。
